...
namespace Microsoft.Protocols.TestTools.StackSdk.RemoteDesktop.Rdpedyc
{
    ...
    public class CompressFactory
    {
        ...
        ///  Constructor, with some initialization.
        /// </summary>
        /// <param name="historyLen"> Maximum match distance / minimum history size </param>
        /// <param name="maxLen">§	Maximum number of uncompressed bytes in a single segment</param>
        ...
        {
            if ((value >= 0) && (value < (Math.Pow(96, width) - 1)))
            {
                IList<byte> encoded = new List<byte>();

                while (value > 0)
                {
                    byte b = (byte)((value % 96) + (int)' ');
                    encoded.Insert(0, b);
                    value = (int)Math.Floor((double)value / 96);
                }
        ...
        {
            int _minStringLength = (int)SEGMENT_PART_SISE.MAX_PACKET_COMPR_TYPE_RDP8_SEGMENT_PART_SIZE;
        ...
        {
            if (header == ((byte)PACKET_COMPR_FLAG.PACKET_COMPR_TYPE_RDP8 | (byte)PACKET_COMPR_FLAG.PACKET_COMPRESSED) ||
                header == ((byte)PACKET_COMPR_FLAG.PACKET_COMPR_TYPE_LITE | (byte)PACKET_COMPR_FLAG.PACKET_COMPRESSED)
                )
            {
                // Data is compressed
                compressData = data;
