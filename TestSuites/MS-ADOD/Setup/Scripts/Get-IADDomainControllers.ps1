...
function Get-IADDomainControllers {
  ...
  if($DCList.Count -eq 0) {
    echo "Unable to find a domain controller for $DomainName domain. Please check whether the domain name is correct." | out-file "c:\MS-ADOD.log.txt" -append
    return $null
  }

  Foreach ($DC in $DCList) {
    if($DC.Partitions.Count -eq 0) {
      echo "No Objects in $DC server" | out-file "c:\MS-ADOD.log.txt" -append
      continue
    }
    $Root = Get-IADSearchRoot -ServerName $DC -ObjectDN $DC.Partitions[0] -UserName $UserName -Password $Password
    if($Root -eq $null) {
      echo "DomainController $DC can not be connected or does not exist." | out-file "c:\MS-ADOD.log.txt" -append
    }
  }
  return $DCList
}