...
## File:           ChangeUserAcctPasswordSAMR.ps1
...
[string]$domainNewUserNewPwd 	= $PtfProp_DomainNewUserNewPwd
...
echo "EXECUTING [ChangeUserAcctPasswordSAMR.ps1]." |Out-File $logFile -Append
...
echo "`$domainNewUserNewPwd         = $domainNewUserNewPwd" |Out-File $logFile -Append
...
trap
{
	...
	Throw "EXECUTE [ChangeUserAcctPasswordSAMR.ps1] FAILED. For more information, please see $logFile."
}
...
if ($domainNewUserNewPwd -eq $null -or $domainNewUserNewPwd -eq "")
{
	Throw "Parameter `$domainNewUserNewPwd NOT found."
}
...
Invoke-Command -Session $RemoteSession -ScriptBlock {param ($pathT) . ($pathT+"\Set-IADUser.ps1")} -ArgumentList $scriptPath
...
Invoke-Command -Session $RemoteSession -ScriptBlock {param ($NewUserNewPwdT) $NewUserNewPwd = $NewUserNewPwdT} -ArgumentList $domainNewUserNewPwd

$NewUser = Invoke-Command -Session $RemoteSession -ScriptBlock {Get-IADUser -Name $NewUserName -SearchRoot $SearchRoot | Set-IADUser -Password $NewUserNewPwd -PasswordNeverExpires}
...
# Ending script
#-------------------------------------------------------------------------------------#
echo "EXECUTE [ChangeUserAcctPasswordSAMR.ps1] FINISHED." | Out-File $logFile -Append
if($NewUser -eq $null) {
  return $false
}
return $true