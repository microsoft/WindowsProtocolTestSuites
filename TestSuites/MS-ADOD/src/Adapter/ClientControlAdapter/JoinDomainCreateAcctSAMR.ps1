...
echo "EXECUTING [JoinDomainCreateAcctSAMR.ps1]." | Out-File $logFile -Append
...
trap
{
	...
	Throw "EXECUTE [JoinDomainCreateAcctSAMR.ps1] FAILED. For more information, please see $logFile."
}
...
#-------------------------------------------------------------------------------------#
# Join Domain by creating a computer account using SAMR
#-------------------------------------------------------------------------------------#
echo "Join Domain by creating a computer account using SAMR..." | Out-File $logFile -Append
$errorcode = Invoke-Command -Session $RemoteSession -ScriptBlock {param ($domainNameT, $userNameT, $passwordT) JoinDomain-CreateAccount-SAMR $domainNameT $userNameT $passwordT} -ArgumentList $fullDomainName, $fullDomainName\$domainAdminUserName, $domainAdminPwd
...
if($errorcode -eq 0) {
	...
	echo "EXECUTE [JoinDomainCreateAcctSAMR.ps1] FINISHED." | Out-File $logFile -Append
	return $true
}
else {
