...
## File:           DetermineUserAcctMembershipSAMR.ps1
## Purpose:        Setup remote session on to the client computer and determine a user account's group membership in AD.
...
echo "EXECUTING [DetermineUserAcctMembershipSAMR.ps1]." |Out-File $logFile -Append
...
Invoke-Command -Session $RemoteSession -ScriptBlock {param ($pathT) . ($pathT+"\Get-IADGroupMembership.ps1")} -ArgumentList $scriptPath

#-------------------------------------------------------------------------------------#
# Determine a user account's group membership in Active Directory
#-------------------------------------------------------------------------------------#
echo "Determine a user account's group membership in Active Directory..." | Out-File $logFile -Append
...
$GroupList = Invoke-Command -Session $RemoteSession -ScriptBlock {Get-IADUser -Name $NewUserName -SearchRoot $SearchRoot | Get-IADGroupMembership}
$GroupList | Out-File $logFile -Append
...
# Ending script
#-------------------------------------------------------------------------------------#
echo "EXECUTE [DetermineUserAcctMembershipSAMR.ps1] FINISHED." | Out-File $logFile -Append
$GroupList | ForEach-Object {$result += "$_;"}
return $result