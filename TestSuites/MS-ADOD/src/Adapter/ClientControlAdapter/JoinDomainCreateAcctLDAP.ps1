...
## File:           JoinDomainCreateAcctLDAP.ps1
...
echo "EXECUTING [JoinDomainCreateAcctLDAP.ps1]." | Out-File $logFile -Append
...
#-------------------------------------------------------------------------------------#
# Join Domain by creating a computer account using LDAP
#-------------------------------------------------------------------------------------#
echo "Join Domain by creating a computer account using LDAP..." | Out-File $logFile -Append
$errorcode = Invoke-Command -Session $RemoteSession -ScriptBlock {param ($domainNameT, $userNameT, $passwordT) JoinDomain-CreateAccount-LDAP $domainNameT $userNameT $passwordT} -ArgumentList $fullDomainName, $fullDomainName\$domainAdminUserName, $domainAdminPwd
