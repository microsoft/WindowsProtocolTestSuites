...
## File:           DeleteUserAcct.ps1
## Purpose:        Setup remote session on to the client computer and delete a user account in AD.
...
echo "EXECUTING [DeleteUserAcct.ps1]." |Out-File $logFile -Append
...
#-------------------------------------------------------------------------------------#
echo "Delete a user account in Active Directory..." | Out-File $logFile -Append
...
$Result = Invoke-Command -Session $RemoteSession -ScriptBlock {Get-IADUser -Name $NewUserName -SearchRoot $SearchRoot | Remove-IADObject}
...
# Ending script
#-------------------------------------------------------------------------------------#
echo "EXECUTE [DeleteUserAcct.ps1] FINISHED." | Out-File $logFile -Append
return $Result