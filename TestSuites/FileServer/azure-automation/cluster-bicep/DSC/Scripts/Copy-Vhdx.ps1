...
function Write-ConfigFailureSignal() {
    ...
    "Execute Copy-Vhdx.ps1 failed, read Copy-Vhdx.ps1.log for detail." | Out-File -FilePath $startSignalFile -Append
}
...
if($null -ne $policy)
{
    .\Write-Info.ps1 "Create Storage QoS policy: Desktop - File Path - $systemDrive\SqosPolicyId.txt" -ForegroundColor Cyan
    Set-Content -Path "$systemDrive\SqosPolicyId.txt" -Value $policy.PolicyId
}
