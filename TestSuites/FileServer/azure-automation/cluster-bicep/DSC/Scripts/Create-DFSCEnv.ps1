...
if ([double]$osVersion -ge [double]"6.2")
{
    ...
    # Only install if not already present (Install-WindowsFeature.ps1 batch may have done this)
    $dfs = Get-WindowsFeature FS-DFS-Namespace -ErrorAction SilentlyContinue
    if ($dfs -and -not $dfs.Installed) {
        Write-Info.ps1 "Installing FS-DFS-Namespace..." -ForegroundColor Yellow
        Add-WindowsFeature FS-DFS-Namespace
    } else {
        Write-Info.ps1 "FS-DFS-Namespace already installed, skipping." -ForegroundColor Green
    }

    $type = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion").InstallationType
    if ($type -ne "Server Core") {
        $dfsMgmt = Get-WindowsFeature RSAT-DFS-Mgmt-Con -ErrorAction SilentlyContinue
        if ($dfsMgmt -and -not $dfsMgmt.Installed) {
