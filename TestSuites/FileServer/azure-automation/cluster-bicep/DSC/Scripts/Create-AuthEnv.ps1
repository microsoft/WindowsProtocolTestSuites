...
$fsrmResult = Install-WindowsFeature FS-Resource-Manager -IncludeManagementTools -ErrorAction SilentlyContinue
    
#----------------------------------------------------------------------------
# Update FSRMClassificationpropertyDefinition
#----------------------------------------------------------------------------
if ($null -ne $fsrmResult -and $fsrmResult.Success) {
    Write-Info.ps1 "Importing FileServerResourceManager module"
    ...
    if (Get-Command Update-FSRMClassificationpropertyDefinition -ErrorAction SilentlyContinue) {
        ...
        Write-Info.ps1 "Update-FSRMClassificationpropertyDefinition cmdlet not available, skipping."
    }
}
else {
    Write-Info.ps1 "FS-Resource-Manager feature not installed or unavailable, skipping FSRM configuration."
}
