...
function CreateFAT32VolumeForFSA()
{
    ...
    $volume = Get-CimInstance -ClassName Win32_Volume | Where-Object { $_.FileSystem -eq "FAT32" -and $_.Label -eq "FAT32" }
    if ($null -eq $volume) {
        Write-Info.ps1 "Create Volume for SMBFAT32Share"
...
# This script assumes Create-SMB2Env.ps1 has been run and created K: volume for ReFS file system.
CreateShareFolderForFSA -Path "K:" -FolderName "SMBReFSShare"
# FAT32 volume on J: is created above by CreateFAT32VolumeForFSA.
...
#----------------------------------------------------------------------------
 ...
 Set-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem -Name RefsDisableLastAccessUpdate -Value 0 
