...
resource driverLinuxExtension 'Microsoft.Compute/virtualMachines/extensions@2023-03-01' = if (driverIsLinux && !empty(clusterPackageZipUrl)) {
  ...
  properties: {
    ...
    protectedSettings: {
      commandToExecute: 'bash -c "set -e; exec > >(tee -a /var/log/cluster-driver-setup.log) 2>&1; echo Starting Driver Computer Setup for Cluster Testing...; mkdir -p /opt/Cluster-Package; zipfile=\\$(find /var/lib/waagent/custom-script/download/0 -maxdepth 1 -name *.zip 2>/dev/null | head -1); if [ -n \\"\\$zipfile\\" ]; then echo Extracting \\$zipfile...; apt-get update -qq && apt-get install -y -qq unzip powershell; unzip -o \\"\\$zipfile\\" -d /opt/Cluster-Package; rm -f \\"\\$zipfile\\"; else echo No zip file found; exit 1; fi; echo Starting Deploy-Driver.ps1 execution...; if [ -f /opt/Cluster-Package/DSC/Deploy-Driver.ps1 ]; then pwsh -ExecutionPolicy Unrestricted -File /opt/Cluster-Package/DSC/Deploy-Driver.ps1 -WorkingPath /opt/Cluster-Package; else echo Deploy-Driver.ps1 not found, skipping configuration; fi; echo Driver Computer extension setup completed"'
    }
  }
}
