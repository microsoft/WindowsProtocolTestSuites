...
<#
.SYNOPSIS
    Pre-reboot feature configuration for the Domain SUT.

.DESCRIPTION
    Installs every disruptive SUT feature in one DSC Script resource. The
    orchestrator then joins the domain and uses one normal member reboot for
    both feature servicing and domain membership.
#>
        ...
        Script DomainSutFeatureBundle {
            ...
            SetScript = {
                ...
                New-ItemProperty -Path 'HKLM:\SOFTWARE\ProtocolTestSuites' `
                    -Name 'DomainSutFeatureBundleAttempted' -Value 1 `
