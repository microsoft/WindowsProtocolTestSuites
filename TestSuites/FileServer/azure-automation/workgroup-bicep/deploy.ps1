...
if (-not $DscPackageZipUrl -and (Test-Path $DscFolderPath)) {
    ...
    if (Test-Path $sharedDscPath) {
        # Root-level scripts (Deploy-Driver.ps1, Driver-Configuration.ps1, etc.)
    ...
    # Download external assets (ParamConfig.json) into the package
    Install-DscPackageAssets -ScriptsFolder $dscScriptsTarget -Scenario 'Workgroup'
    ...
    # Copy generated Config.json into DSC\Scripts (overwrite template so utility
    ...
    $actualDscPackageZipUrl = Send-BlobWithSasUrl `
        -FilePath $tempZipPath -BlobName "DSC-Package.zip" `
