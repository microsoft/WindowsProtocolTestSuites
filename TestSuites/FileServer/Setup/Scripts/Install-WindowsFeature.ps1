...
$signalfile = "$scriptPath\Install-WindowsFeature.Completed.signal"
if (Test-Path $signalfile) {
    .\Write-Info.ps1 "Features Installed" -ForegroundColor Green
    Stop-Transcript
    return $true
}
...
if((Get-WmiObject win32_computersystem).partofdomain -and (Get-WindowsFeature "AD-Domain-Services").InstallState -eq "Installed")
{
...
elseif([double]$osVersion -ge [double]"6.2")
{
    ...
    {
        .\Write-Info.ps1 "Install Windows Features for Storage Server" -ForegroundColor Yellow
