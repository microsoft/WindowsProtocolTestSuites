...
namespace Microsoft.Protocols.TestSuites.FileSharing.FSA.TestSuite
{
    public partial class AlternateDataStreamTestCases : PtfTestClassBase
    {
        ...
        public void AlternateDataStream_RenameStream_File()
        {
        ...
        public void AlternateDataStream_RenameStream_Dir()
        {
        ...
        private void AlternateDataStream_RenameStream(FileType fileType)
        {
            ...
            // Step 9: Rename the Alternate Data Stream <Stream2> to a new stream name <Stream3>
            string streamName3 = this.fsaAdapter.ComposeRandomFileName(8);
            streamList.Remove(":" + streamName2 + ":$DATA");
            streamList.Add(":" + streamName3 + ":$DATA", 4096);

            BaseTestSite.Log.Add(LogEntryKind.TestStep, "9. Rename the Alternate Data Stream with name: " + streamName2 + " to a new stream name: " + streamName3);
            ...
            // Step 10: List the Alternate Data Streams by querying FileStreamInformation
            BaseTestSite.Log.Add(LogEntryKind.TestStep, "10. List the Alternate Data Streams on this file.");
            status = this.fsaAdapter.QueryFileInformation(
                FileInfoClass.FILE_STREAM_INFORMATION,
                outputBufferSize,
                out byteCount,
            ...
            // Step 11: Verify the FileStreamInformation entry list from the outputBuffer returned by the query
            fileStreamInformations = ParseFileStreamInformations(outputBuffer);

            BaseTestSite.Log.Add(LogEntryKind.TestStep, "11. Verify fields in each FileStreamInformation entry.");
            foreach (FileStreamInformation fileStreamInformation in fileStreamInformations)
            {
                string streamName = Encoding.Unicode.GetString(fileStreamInformation.StreamName);
                KeyValuePair<string, long> streamListElement = streamList.SingleOrDefault(x => x.Key.Equals(streamName));
