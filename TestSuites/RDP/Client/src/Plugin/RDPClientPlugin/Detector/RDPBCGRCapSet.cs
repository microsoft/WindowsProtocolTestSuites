...
namespace Microsoft.Protocols.TestManager.RDPClientPlugin
{
    public class RdpbcgrCapSet
    {
        ...
        /// <summary>
        /// Method to convert struct to byte[]
        /// </summary>
        /// <param name="structPrepare">The struct prepare to convert</param>
        /// <returns>the got byte array converted from struct</returns>
        internal static byte[] StructToBytes(object structPrepare)
        {
            ...
            try
            {
                int size = Marshal.SizeOf(structPrepare.GetType());
                ptr = Marshal.AllocHGlobal(size);
                buffer = new byte[size];
                Marshal.StructureToPtr(structPrepare, ptr, false);
        ...
        {
            TS_BITMAPCACHE_HOSTSUPPORT_CAPABILITYSET bitmapHostsupport =
                new TS_BITMAPCACHE_HOSTSUPPORT_CAPABILITYSET();
            bitmapHostsupport.capabilitySetType = capabilitySetType_Values.CAPSTYPE_BITMAPCACHE_HOSTSUPPORT;
            bitmapHostsupport.cacheVersion = cacheVersion_Values.V1;
            bitmapHostsupport.pad1 = 0;
            bitmapHostsupport.pad2 = 0;
            bitmapHostsupport.lengthCapability = (ushort)Marshal.SizeOf(bitmapHostsupport);
            return bitmapHostsupport;
        }
