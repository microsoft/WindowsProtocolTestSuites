...
## Purpose:        Configure Read-Only Domain Controller in Local Domain for Active Directory Family Test Suite.
...
# 5.  Promote Domain Controller as a Read-Only Domain Controller
...
# 5.  [MS-DRSR] Set computer password for this read-only domain controller
# 6.  [MS-DRSR] Replicate default naming context from PDC to RODC
...
Function Config-Phase3()
{
    ...
    # Replicate default naming context from PDC to RODC
    Write-ConfigLog "Replicating default naming context from PDC to RODC..." -ForegroundColor Yellow
    .\WaitFor-ComputerReady.ps1 -computerName $Parameters["replicasourcedc"] -usr $Parameters["username"] -pwd $Parameters["password"]
