...
namespace Microsoft.Protocols.TestSuites.ActiveDirectory.Drsr
{
    public partial class KerberosTestClient : KerberosClient
    {
            ...
            : base(domain, cName, password, accountType, kdcAddress, kdcPort, transportType, oidPkt, salt)
        {
            ...
            Asn1SequenceOf<Microsoft.Protocols.TestTools.StackSdk.Security.KerberosLib.KerbInt32> etype = new Asn1SequenceOf<Microsoft.Protocols.TestTools.StackSdk.Security.KerberosLib.KerbInt32>(etypes);
        ...
        public void SendAsRequest(KdcOptions kdcOptions, Asn1SequenceOf<PA_DATA> seqPaData)
        {
            ...
            this.testSite.Log.Add(LogEntryKind.Comment, "Send AS Request.");
        }
        ...
        public void SendAsRequest(KdcOptions kdcOptions, Asn1SequenceOf<PA_DATA> seqPaData, long pvno)
        {
            string sName = KerberosConstValue.KERBEROS_SNAME;
            string domain = this.Context.Realm.Value;
            PrincipalName sname =
            ...
            ApOptions apOptions)
        {
            ...
            this.SendPdu(asRequest);
            this.testSite.Log.Add(LogEntryKind.Comment, "Send AS Request with FAST PA-DATA.");
        }
        ...
        public KerberosAsResponse ExpectAsResponse()
        {
            ...
            this.testSite.Assert.IsInstanceOfType(responsePdu, typeof(KerberosAsResponse), "Response type mismatch");

            KerberosAsResponse response = responsePdu as KerberosAsResponse;
            this.testSite.Log.Add(LogEntryKind.Comment, " Receive  AS request");
        ...
        public KerberosKrbError ExpectKrbError()
        {
            ...
            testSite.Log.Add(LogEntryKind.Comment, "Receive Kerberos Error.");
            return krbError;
        }
        ...
        public KerberosKrbError ExpectPreauthRequiredError(out METHOD_DATA eData)
        {
            ...
            this.testSite.Log.Add(LogEntryKind.Comment, "Receive preauthentication required error.");
            ...
            AuthorizationData dataInEncAuthData = null)
        {
            ...
            {
                tempPaData = new Asn1SequenceOf<PA_DATA>(new PA_DATA[seqPadata.Elements.Length + 1]);
                Array.Copy(seqPadata.Elements, tempPaData.Elements, seqPadata.Elements.Length);
                tempPaData.Elements[seqPadata.Elements.Length] = paTgsReq;
            }
        ...
        public KerberosTgsResponse ExpectTgsResponse(KeyUsageNumber usage = KeyUsageNumber.TGS_REP_encrypted_part)
        {
            ...
            this.testSite.Log.Add(LogEntryKind.Comment, "Receive TGS response.");
            this.testSite.Assume.IsNotNull(Context.ReplyKey, "Reply key should not be null.");
