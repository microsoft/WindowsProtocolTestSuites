...
Function Phase1
{
    ...
    if(Test-Path -Path $dataFile)
    {
        try
        {
	        ...
	        $logFile	= $logPath + "\Config-DC02.ps1.log"
...
Function Phase3
{
    ...
    # Create Wscript.shell for UI automation
    $AUKEY = New-Object -ComObject Wscript.shell

    # Open Group Policy Browser
    get-process | where {$_.ProcessName -like "mmc*"} | foreach {$_.Kill()}
    GPME.msc

    # Navigate to Default Domain Policy and Open Group Policy Management Editor
    SLEEP 10
    ...
    # Under Computer Configuration > Policies > Windows Settings > Security > File System > Central Access Policy
    # Add "FileAccessPolicy" to Central Access Policy
    SLEEP 10
    ...
    SLEEP 10
    $AUKEY.SendKeys("+^{END}")     #select all central access policies
    SLEEP 10
    	
    $AUKEY.SendKeys("%A")          #Press ALT+A to Add First Access Policy 
    SLEEP 3
    ...
    $endPointPath = "$env:SystemDrive\MicrosoftProtocolTests\MS-AZOD\OD-Endpoint"
    $installed = Get-ChildItem -Path 'HKLM:\SOFTWARE\Wow6432Node\Microsoft\ProtocolTestSuites\MS-AZOD-OD-*'
    $version = $installed.Name.Substring($installed.Name.Length-10,10)
...
$logFile =  "$rootPath\Config-DC02.ps1.log"
