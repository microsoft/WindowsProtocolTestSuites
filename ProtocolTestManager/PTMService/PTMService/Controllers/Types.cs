                ...
                if (r.Categories.Length > 0 && mappingTable.ContainsKey(r.Categories.FirstOrDefault()))
                {
                    ...
                    if (newSelectedHiddenNodes.Count() > 0 && !r.Name.Contains('%') && selectedRules != null)
                    {
                        ...
                        if (selectedRule != null)
                        {
                            ...
                            var rule = selectedRule.Rules.Where(i => i.Name.Contains(parentRuleName) && (i.Name.Contains('.') ? i.Name.Remove(0, i.Name.LastIndexOf('.') + 1) : i.Name) == r.Name && Enumerable.SequenceEqual(i.Categories, r.Categories)).FirstOrDefault();
                            if (rule != null)
                            {
